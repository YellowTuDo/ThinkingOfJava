<div style="font-size: 15px;line-height: 1.8;word-spacing: 2px;letter-spacing: 2px;font-family: 'Helvetica Neue', Helvetica, 'Hiragino Sans GB', 'Microsoft YaHei', Arial, sans-serif;background-image: linear-gradient(90deg, rgba(50, 0, 0, 0.05) 3%, rgba(0, 0, 0, 0) 3%), linear-gradient(360deg, rgba(50, 0, 0, 0.05) 3%, rgba(0, 0, 0, 0) 3%);background-size: 20px 20px;background-position: center center;" id="output_wrapper_id" class="output_wrapper"><blockquote style="line-height: inherit; display: block; padding: 15px 15px 15px 1rem; font-size: 0.9em; margin: 1em 0px; color: rgb(0, 0, 0); border-left: 5px solid rgb(239, 112, 96); background: rgb(239, 235, 233); overflow: auto; overflow-wrap: normal; word-break: normal;">一切不可能，终将化为寻常。</br>
 喜欢一个人就说喜欢，心存感恩就说谢谢；说反话并不会引起别人的关注，只会把人越推越远。。</div></br>
...
#### 1.软件结构
	    1.1C/S  （Client/Server），是指客户端和服务器结构
          B/S  （Browser/Serve），是指浏览器和服务器结构
	    
	    1.2两种架构各有优势，但无论那种架构，都离不开网络的支持。
#### 2.网络编程三要素
        1、通信协议:计算机网络通信必须遵守的规则。
		UDP协议：面向无连接、效率高、不保证数据完整
		TCP协议：面向连接、效率低、保证数据完整
...
<div align=center><img height="360px" src="https://user-gold-cdn.xitu.io/2019/12/4/16ecf7653708f034?w=706&h=403&f=png&s=84262"></img></div></br> 
...
<div align=center><img height="460px" src="https://user-gold-cdn.xitu.io/2019/12/4/16ecf814b3ee7132?w=888&h=648&f=png&s=353125"></img></div></br> </br>
...
实际差异
        ...
        1.GET和POST本质上就是TCP链接，并无差别。但是由于HTTP的规定和浏览器/服务器的限制，导致他们在应用
        过程中体现出一些不同。
        2.GET产生一个TCP数据包；POST产生两个TCP数据包。
        3.GET与POST都有自己的语义，不能随便混用。
        4.在网络环境好的情况下，发一次包的时间和发两次包的时间差别基本可以无视。而在网络环境差的情况下，
...
##  <div style="border-bottom:1px solid #DEDEDE">输入xxURL按下回车发生了什么?</div>
...
6.浏览器根据自身的执行机制解析页面；（浏览器的执行机制？重绘？重排？......）浏览器解析页面时，会找到每一个文件夹（css、js、html、img......），<code style=" background-color: #fff5f5;color: #ff502c;font-size: .87em;padding: .065em .4em;">每一个文件夹下的资源会重新走到第二步</code>，去找到相应的服务器，然后一步步执行。
